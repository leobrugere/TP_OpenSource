...
resource "azurerm_virtual_machine" "vm_vm_opSo4" {
    count                 = "${var.count}"
    name                  = "vm_opSo4"
    location              = "${var.location}"
    resource_group_name   = "${var.rsg-name}"
    network_interface_ids = ["${element(azurerm_network_interface.nic_vm_opSo4.*.id, count.index)}"] 
    vm_size               = "Standard_A2"


    storage_os_disk {
        name              = "${var.vm-osdisk}_vm_opSo4"
        caching           = "ReadWrite"
